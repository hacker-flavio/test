...
app.use(
  session({
    genid: function (req) {
      return uuid.v4(); // generate a new session ID using a UUID generator
    },
    key: "userId",
...
async function fetchTopMemes(req) {
  ...
  req.session.save((err) => {
    if (err) {
      ...
      console.log("session saved to mongdb");
    }
  });
...
app.get("/memes", (req, res) => {
  ...
  if (req.session.memes) {
    ...
    return req.session.memes;
  } else {
    console.log(req.session);
  }

  fetchTopMemes(req).then((memes) => {
